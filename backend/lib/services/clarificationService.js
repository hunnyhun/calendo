...
exports.analyzeCompleteness = analyzeCompleteness;
exports.identifyMissingFields = identifyMissingFields;
exports.generateClarifyingQuestion = generateClarifyingQuestion;
exports.assessConfidence = assessConfidence;
/**
 * Analyze completeness of extracted data against schema
 */
function analyzeCompleteness(data, schema, type) {
    const missingFields = [];
    let extractedData = {};
    try {
 ...
 */
function identifyMissingFields(data, schema, type) {
 ...
 */
function generateClarifyingQuestion(missingField, context, type) {
    const field = missingField.field;
    const path = missingField.path;
    // Generate context-aware questions
    if (type === 'habit') {
        return generateHabitQuestion(field, path, context);
    }
    else {
        return generateTaskQuestion(field, path, context);
    }
}
/**
 * Assess confidence score
 */
function assessConfidence(data, schema, type) {
    const analysis = analyzeCompleteness(data, schema, type);
...
function isCriticalField(path, type) {
...
function getRequiredFieldsCount(type) {
    // Approximate count of required fields
    return type === 'habit' ? 15 : 8;
}
function generateHabitQuestion(field, path, context) {
    const questions = {
...
function generateTaskQuestion(field, path, context) {
    const questions = {
