...
exports.generateHabitJson = generateHabitJson;
exports.generateTaskJson = generateTaskJson;
exports.validateJson = validateJson;
const prompts_1 = require("@langchain/core/prompts");
const habitSchema_1 = require("../schemas/habitSchema");
const taskSchema_1 = require("../schemas/taskSchema");
/**
 * Generate habit JSON using function-calling (if enabled) or structured output
 */
async function generateHabitJson(conversationContext, sessionState, config) {
    try {
        let jsonData;
        if (config.useFunctionCalling) {
            // Use function-calling for deterministic JSON
            jsonData = await generateWithFunctionCalling(conversationContext, 'habit', config);
        }
        else {
            // Use structured output with prompt
            jsonData = await generateWithStructuredOutput(conversationContext, 'habit', config);
        }
        // Normalize the data before validation
        jsonData = normalizeHabitJson(jsonData);
        // Always validate with zod
        const validationResult = habitSchema_1.habitSchema.safeParse(jsonData);
 ...
 */
async function generateTaskJson(conversationContext, sessionState, config) {
    try {
        let jsonData;
        if (config.useFunctionCalling) {
            // Use function-calling for deterministic JSON
            jsonData = await generateWithFunctionCalling(conversationContext, 'task', config);
        }
        else {
            // Use structured output with prompt
            jsonData = await generateWithStructuredOutput(conversationContext, 'task', config);
        }
        // Always validate with zod
        const validationResult = taskSchema_1.taskSchema.safeParse(jsonData);
        if (!validationResult.success) {
 ...
 */
async function generateWithFunctionCalling(conversationContext, type, config) {
 ...
 */
async function generateWithStructuredOutput(conversationContext, type, config) {
 ...
 */
function buildHabitGenerationPrompt() {
 ...
 */
function normalizeHabitJson(jsonData) {
    ...
    if (normalized.low_level_schedule) {
        ...
        if (normalized.low_level_schedule.program && Array.isArray(normalized.low_level_schedule.program)) {
            normalized.low_level_schedule.program = normalized.low_level_schedule.program.map((programItem) => {
                ...
                if (programItem.days_indexed && Array.isArray(programItem.days_indexed)) {
                    programItem.days_indexed = programItem.days_indexed.map((item, idx) => {
                ...
                if (programItem.weeks_indexed && Array.isArray(programItem.weeks_indexed)) {
                    programItem.weeks_indexed = programItem.weeks_indexed.map((item, idx) => {
                        if (typeof item === 'number') {
                            // Convert number to proper object structure
                            return {
                                index: item,
                ...
                if (programItem.months_indexed && Array.isArray(programItem.months_indexed)) {
                    programItem.months_indexed = programItem.months_indexed.map((item, idx) => {
                        if (typeof item === 'number') {
                            // Convert number to proper object structure
                            return {
                                index: item,
 ...
 */
function validateJson(jsonData, type) {
    // Normalize before validation
    const normalized = type === 'habit' ? normalizeHabitJson(jsonData) : jsonData;
    const schema = type === 'habit' ? habitSchema_1.habitSchema : taskSchema_1.taskSchema;
