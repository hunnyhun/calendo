...
exports.processChatMessageV3 = exports.generateHabitJson = exports.recordShareImport = exports.getSharedItem = exports.createShareLink = exports.processChatMessageV2 = exports.getTasksV2 = exports.getHabitsV2 = exports.getChatHistoryV2 = void 0;
...
const jsonGenerator_1 = require("./services/jsonGenerator");
...
async function checkUserAuthentication(auth) {
    ...
    if (standardProvider === 'anonymous') {
        ...
        throw new https_1.HttpsError('permission-denied', 'Anonymous authentication is not allowed. Please sign in with Google or Apple.');
    }
    ...
    if (standardProvider !== 'google.com' && standardProvider !== 'apple.com') {
        ...
        throw new https_1.HttpsError('permission-denied', 'Only Google or Apple authentication is allowed.');
    }
            ...
            const tasks = snapshot.docs.map(doc => {
                ...
                if (data.task_schedule && typeof data.task_schedule === 'object') {
                    ...
                    if (taskSchedule.steps && Array.isArray(taskSchedule.steps)) {
                        taskSchedule.steps = taskSchedule.steps.map((step) => {
...
exports.createShareLink = (0, https_1.onCall)({
    ...
    try {
        ...
        if (!type || (type !== 'habit' && type !== 'task')) {
            throw new https_1.HttpsError('invalid-argument', 'Type must be "habit" or "task"');
        }
        ...
        if (!itemDoc.exists) {
            throw new https_1.HttpsError('not-found', `${type} not found`);
        }
        const itemData = itemDoc.data();
        if (!itemData) {
            throw new https_1.HttpsError('not-found', `${type} data not found`);
        }
 ...
 */
exports.getSharedItem = (0, https_1.onCall)({
    region: 'us-central1',
    ...
    try {
        ...
        if (!shareId || typeof shareId !== 'string') {
            throw new https_1.HttpsError('invalid-argument', 'shareId is required');
        }
        ...
        if (!sharedItemData) {
            throw new https_1.HttpsError('not-found', 'Share link data not found');
        }
        // Check expiration (if implemented)
        if (sharedItemData.expiresAt) {
            const expiresAt = sharedItemData.expiresAt.toDate();
            if (expiresAt < new Date()) {
                throw new https_1.HttpsError('not-found', 'Share link has expired');
            }
        }
        // Increment view count
        await sharedItemRef.update({
            viewCount: firestore_1.FieldValue.increment(1)
        });
        ...
        throw new https_1.HttpsError('internal', 'Failed to retrieve shared item');
    }
});
/**
 ...
 */
exports.recordShareImport = (0, https_1.onCall)({
    region: 'us-central1',
    enforceAppCheck: true,
}, async (request) => {
    try {
        ...
        const sharedItemDoc = await sharedItemRef.get();
        if (!sharedItemDoc.exists) {
            throw new https_1.HttpsError('not-found', 'Share link not found');
        }
        const sharedItemData = sharedItemDoc.data();
        if (!sharedItemData) {
            throw new https_1.HttpsError('not-found', 'Share link data not found');
        ...
        if (!alreadyImported) {
            ...
            updateData.importCount = firestore_1.FieldValue.increment(1);
            updateData.importedBy = firestore_1.FieldValue.arrayUnion(uid);
        }
        ...
        throw new https_1.HttpsError('internal', 'Failed to record share import');
    }
});
...
${conversation}`;
        ...
        if (habitJson.high_level_schedule && !habitJson.high_level_schedule.milestones) {
            ...
            throw new https_1.HttpsError('internal', 'high_level_schedule must contain milestones array');
        }
        if (habitJson.high_level_schedule?.milestones && habitJson.high_level_schedule.milestones.length === 0) {
            console.error(`${logPrefix} high_level_schedule.milestones is empty`);
            throw new https_1.HttpsError('internal', 'high_level_schedule.milestones must contain at least one milestone');
        }
        // Add Firebase timestamps
        const now = new Date().toISOString();
        habitJson.created_at = now;
 ...
 */
exports.processChatMessageV3 = (0, https_1.onRequest)({
    region: 'us-central1',
    secrets: [geminiSecretKey]
}, async (req, res) => {
    try {
        ...
        if (conversationId) {
            sessionState = await (0, sessionManager_1.getSessionState)(uid, conversationId);
        }
        if (!sessionState) {
            sessionState = await (0, sessionManager_1.createSessionState)(uid, finalConversationId, {
                messages: [],
                chatMode: chatMode,
                intent: 'unknown',
                confidence: 0,
        ...
        const langChainModel = (0, langchainService_1.createLangChainModel)({
        ...
        if (isStreamingRequest) {
            ...
            let fullResponse = '';
            for await (const chunk of (0, langchainService_1.executeConversationStream)(langChainModel, message, { ...sessionState, messages: updatedMessages }, chatMode)) {
                fullResponse += chunk;
            ...
            conversationResponse = await (0, langchainService_1.executeConversation)(langChainModel, message, { ...sessionState, messages: updatedMessages }, chatMode);
        }
        ...
        const indicatesReadiness = responseLower.includes('ready') ||
        ...
        const shouldGenerateJson = (finalIntent === 'habit' || finalIntent === 'task' || chatMode === 'habit' || chatMode === 'task') &&
        ...
        if (shouldGenerateJson) {
            ...
            const conversationContext = finalMessages
                ...
                .join('\n\n');
            if (chatMode === 'habit') {
                const result = await (0, jsonGenerator_1.generateHabitJson)(conversationContext, sessionState, jsonConfig);
                if (result.success && result.json) {
                    generatedJson = result.json;
                    responseText += `\n\n**Your Personalized Habit Program:**\n\`\`\`json\n${JSON.stringify(generatedJson, null, 2)}\n\`\`\``;
                    // Clear session state after successful generation
                    await (0, sessionManager_1.clearSessionState)(uid, finalConversationId);
                }
                else {
                ...
                const result = await (0, jsonGenerator_1.generateTaskJson)(conversationContext, sessionState, jsonConfig);
                if (result.success && result.json) {
                    generatedJson = result.json;
                    responseText += `\n\n**Your Task Breakdown:**\n\`\`\`json\n${JSON.stringify(generatedJson, null, 2)}\n\`\`\``;
                    // Clear session state after successful generation
        ...
        let conversationTitle = sessionState.title;
        ...
        await (0, sessionManager_1.updateSessionState)(uid, finalConversationId, sessionUpdates);
        // Send response
        sendResponse({
            response: responseText,
