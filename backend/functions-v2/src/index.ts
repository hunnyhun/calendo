...
async function ensureQueueExists(): Promise<boolean> {
    try {
        ...
        try {
            ...
            const tasks = snapshot.docs.map(doc => {
                ...
                if (data.steps && Array.isArray(data.steps)) {
                    processedData.steps = data.steps.map((step: any) => {
        ...
        for (const userDoc of usersSnapshot.docs) {
            ...
            try {
                ...
                const tasksToSchedule: Array<{
                    type: 'notification_morning' | 'notification_evening';
                    windowStart: number;
                    windowEnd: number;
                }> = [
            ...
            try {
                ...
                if (isPremium) {
                    ...
                    await userRef.update({ 
                        totalQuotesReceived: FieldValue.increment(1)
                    });
                    ...
                    await userRef.update({ 
                        lifetimeQuoteCount: FieldValue.increment(1)
                    });
...
export const updateUserProfile = onCall({
    ...
    try {
        ...
        const input = (request.data || {}) as Partial<UserProfile>;
        // Basic sanitization: keep only known keys
        const allowedKeys: (keyof UserProfile)[] = ['name','age','gender','goals','intentions','preferredTone','experienceLevel','sufferingDuration','hasCompletedProfileSetup'];
        const profile: Partial<UserProfile> = {};
        for (const key of allowedKeys) {
            if (key in input) {
                (profile as any)[key] = (input as any)[key];
            }
        }
        await db.collection('users').doc(uid).set({ profile, lastActive: FieldValue.serverTimestamp() }, { merge: true });
...
export const scheduleHabitReminders = onCall({
    region: 'us-central1',
    enforceAppCheck: true,
}, async (request) => {
    ...
    try {
        ...
        if (isAnonymous) {
            throw new HttpsError('permission-denied', 'Anonymous users cannot schedule habit reminders.');
        }
        
        const { habitId, reminders } = request.data;
        
        if (!habitId || !reminders || !Array.isArray(reminders)) {
            throw new HttpsError('invalid-argument', 'habitId and reminders array are required.');
        }
        ...
        console.error(`${logPrefix} Error:`, error);
        if (error instanceof HttpsError) throw error;
        throw new HttpsError('internal', 'Failed to schedule habit reminders');
    }
});

// Habit Reminder Task Handler
export const sendHabitReminderTaskHandler = onRequest({
    region: 'us-central1',
}, async (req, res): Promise<void> => {
    ...
    try {
        ...
        for (const deviceDoc of devicesSnapshot.docs) {
            ...
            try {
                await deviceDoc.ref.update({ badgeCount: FieldValue.increment(1) });
                        ...
                        if (!resp.success) {
                            ...
                            if (errorCode === 'messaging/registration-token-not-registered' || 
                                ...
                                cleanupPromises.push(
                                    db.collection('users').doc(userId).collection('devices').doc(failedToken).delete()
